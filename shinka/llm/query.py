...
def sample_model_kwargs(
    ...
    if kwargs_dict["model_name"] in (REASONING_GEMINI_MODELS):
        ...
        if (
            ...
            or kwargs_dict["model_name"] in OPENROUTER_MODELS
        ):
            kwargs_dict["max_tokens"] = random.choice(max_tokens)
        else:
            kwargs_dict["max_output_tokens"] = random.choice(max_tokens)
...
def query(
    ...
    **kwargs,
    ...
    """Query the LLM."""
    original_model_name = model_name  # Keep original for ollama check
    ...
    elif original_model_name.startswith("ollama-"):
        # Ollama uses standard chat completions API
